# CLAUDE.md — sekce Software Design
...
Patterny se přidávají postupně, ad hoc podle toho, které tým zrovna považuje za důležité. Sekce proto nikdy není „hotová“ a nekompletnost není chyba.

## Jazyková konvence

Platí [konvence celého repozitáře](../CLAUDE.md#jazyková-konvence--dodržuj-bez-výjimky): obsah česky, kód anglicky, komunikace s uživatelem slovensky. Navíc pro tuhle sekci:
...
Cesty níž jsou relativní k `SoftwareDesign/`.
...
Platí [pravidla psaní pro celý repozitář](../CLAUDE.md#psaní-obsahu--platí-v-celém-repozitáři) — od problému, ne od definice; *Kdy nepoužít* a *Časté chyby* stejně důležité jako řešení; konkrétně, ne obecně. Navíc pro patterny:

- Doménový příklad je napříč patterny **jednotný — e-shop / objednávky** — aby juniorovi odpadalo přepínání kontextu.
- **Opakovaný pojem patří do [`Glossary.md`](Glossary.md)** a odkazuje se na kotvu (idempotence, invariant, eventuální konzistence…). Jednorázový vysvětli na místě jednou větou. **Pojem, který má vlastní pattern, do slovníčku nepatří** — odkazuje se rovnou na ten dokument.
- **Sekce *V praxi* uvádí jen veřejně ověřitelné věci** (Symfony, Doctrine, PSR, knihovny) — ne to, jak je to zařízené „u nás“.
